...
public class DecodeExample {
  ...
  public static void decode(byte[] oggInput) throws IOException {
    byteArrayOutputStream = new ByteArrayOutputStream();
    ByteArrayInputStream input = new ByteArrayInputStream(oggInput);
    ...
    while(true){ // we repeat if the bitstream is chained
      ...
      // decoding
      {
        ...
        for(int j=0; j<ptr.length; j++){
          ...
          if (comment.contains("loop start=")) {
            loopStart = Integer.parseInt(comment.replace("loop start=", "").trim());
          }
          if (comment.contains("loop end=")) {
            loopEnd = Integer.parseInt(comment.replace("loop end=", "").trim());
          }
        }
