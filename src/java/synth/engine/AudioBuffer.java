...
/**
 * A class for small buffers of samples in linear, 64-bit floating point format.
 * <p>
 * Specifications:
 * <ol>
 ...
 * </ol>
 * <p>
 * When a cascade of AudioInputStreams use AudioBuffer for processing, they may
 * implement the interface FloatSampleInput. This signals that this stream may
 * provide float buffers for reading. The data is <i>not</i> converted back to
 * bytes, but stays in a single buffer that is passed from stream to stream. For
 * that serves the read(AudioBuffer) method, which is then used as replacement
 * for the byte-based read functions of AudioInputStream.<br>
 ...
 * instance of AudioBuffer and initializes it with the number of samples
 * requested in the read(byte[]) method.
 * <li>It queries pcmAIS1 for the FloatSampleInput interface. As it implements
 * it, pcmAIS2 calls the read(AudioBuffer) method of pcmAIS1.
 ...
 * AudioBuffer passed to it. It calls the read(byte[]) method of auAIS.
 ...
 * </ol>
 * <p>
 * To summarize, here are some advantages when using a AudioBuffer for
 * streaming:
 * <ul>
 ...
 * is generally done in-place. The same instance of a AudioBuffer may be used
 * from the original data source to the final data sink.
 * </ul>
 * <p>
 ...
 * instance of AudioBuffer. Some converters may decrease the sample count (e.g.
 * sample rate converter) and delete channels (e.g. PCM2PCM converter). So,
 * processing of one block will decrease both. For the next block, all starts
 * from the beginning. With the lazy mechanism, all double arrays are only
...
public class AudioBuffer {
	...
	// contains for each channel a double array
	...
	private double ditherBits = ConversionTool.DEFAULT_DITHER_BITS;
	...
	/**
	 * Create an empty AudioBuffer with the specified number of channels,
	...
	/**
	 * Creates a new instance of AudioBuffer and initializes it with audio data
	 * given in the interleaved byte array <code>buffer</code>.
	 */
	public AudioBuffer(byte[] buffer, int offset, int byteCount,
			AudioFormat format) {
	...
	protected void init(int aChannelCount, int aSampleCount, double aSampleRate) {
		init(aChannelCount, aSampleCount, aSampleRate, LAZY_DEFAULT);
	}

	protected void init(int newChannelCount, int aSampleCount,
			double aSampleRate, boolean lazy) {
		if (newChannelCount < 0 || aSampleCount < 0) {
			throw new IllegalArgumentException(
					"invalid parameters in initialization of AudioBuffer.");
		}
		setSampleRate(aSampleRate);
		if (getSampleCount() != aSampleCount
				|| this.channelCount != newChannelCount) {
			createChannels(newChannelCount, aSampleCount, lazy);
		}
	}
	...
	private final void createChannels(int newChannelCount, int aSampleCount,
			boolean lazy) {
		// shortcut
		if (lazy && newChannelCount <= channelCount
				&& aSampleCount <= this.sampleCount) {
	 ...
	 * AudioBuffer's sample count will be set to
	...
	public void initFromByteArray(byte[] buffer, int offset, int byteCount,
		...
		if (offset + byteCount > buffer.length) {
			throw new IllegalArgumentException(
					"AudioBuffer.initFromByteArray: buffer too small.");
		}
		...
		originalFormatType = ConversionTool.getFormatType(format);

		ConversionTool.byte2double(buffer, offset, channels, 0, sampleCount,
				format);
	}
	 ...
	 */
	public void initFromAudioBuffer(AudioBuffer source) {
	 ...
	 */
	public void reset(int aChannels, int aSampleCount, double aSampleRate) {
		init(aChannels, aSampleCount, aSampleRate, false);
	}
	...
	public int getByteArrayBufferSize(AudioFormat format, int lenInSamples) {
		...
		ConversionTool.getFormatType(format);
	...
	public int convertToByteArray(int readOffset, int lenInSamples,
		...
		if (writeOffset + byteCount > buffer.length) {
			throw new IllegalArgumentException(
					"AudioBuffer.convertToByteArray: buffer too small.");
		}
		if (format.getSampleRate() != getSampleRate()) {
			throw new IllegalArgumentException(
					"AudioBuffer.convertToByteArray: different samplerates.");
		}
		if (format.getChannels() != this.channelCount) {
			throw new IllegalArgumentException(
					"AudioBuffer.convertToByteArray: different channel count.");
		}
		ConversionTool.double2byte(channels, readOffset, buffer, writeOffset,
				lenInSamples, format,
				getConvertDitherBits(ConversionTool.getFormatType(format)));
	 ...
	 * <code>keepOldSamples</code> is false, existing samples are discarded
	...
	public void changeSampleCount(int newSampleCount, boolean keepOldSamples) {
		...
		// shortcut for one channel
		if (channelCount == 1) {
			double[] ch = getChannel(0);
			if (ch.length < newSampleCount) {
				double[] newCh = new double[newSampleCount];
		...
		init(channelCount, newSampleCount, getSampleRate());
		if (keepOldSamples) {
			// copy old channels and eventually silence out new samples
			int copyCount =
					newSampleCount < oldSampleCount ? newSampleCount
							: oldSampleCount;
			for (int ch = 0; ch < channelCount; ch++) {
				double[] oldSamples = (double[]) oldChannels[ch];
				double[] newSamples = (double[]) channels[ch];
	...
	public void makeSilence() {
		...
		for (int ch = 0; ch < localChannelCount; ch++) {
			makeSilence((double[]) channels[ch]);
		}
	}

	public void makeSilence(int channel) {
		double[] samples = getChannel(channel);
		makeSilence(samples);
	}

	protected void makeSilence(double[] samples) {
	...
	public void insertChannel(int index, boolean silent, boolean lazy) {
		...
		if (newChannel == null) {
			newChannel = new double[getSampleCount()];
		}
	...
	public void removeChannel(int channel, boolean lazy) {
		double[] toBeDeleted = (double[]) channels[channel];
	...
	public void copyChannel(int sourceChannel, int targetChannel) {
		double[] source = getChannel(sourceChannel);
		double[] target = getChannel(targetChannel);
		System.arraycopy(source, 0, target, 0, getSampleCount());
	}

	/**
	 * Copies data inside all channels. When the 2 regions overlap, the behavior
	 * is not specified.
	 */
	...
	public void copy(int channel, int sourceIndex, int destIndex, int length) {
		double[] data = getChannel(channel);
	...
	public void expandChannel(int targetChannelCount) {
		...
		if (getChannelCount() != 1) {
			throw new IllegalArgumentException(
					"AudioBuffer: can only expand channels for mono signals.");
		}
	...
	public void mixDownChannels() {
		double[] firstChannel = getChannel(0);
		int sampleCount1 = getSampleCount();
		int channelCount1 = getChannelCount();
		for (int ch = channelCount1 - 1; ch > 0; ch--) {
			double[] thisChannel = getChannel(ch);
	...
	public void mix(AudioBuffer source) {
		...
		int channelCount1 = getChannelCount();
		if (channelCount1 > source.getChannelCount()) {
			channelCount1 = source.getChannelCount();
		}
		for (int ch = 0; ch < channelCount1; ch++) {
			double[] thisChannel = getChannel(ch);
			double[] otherChannel = source.getChannel(ch);
			for (int i = 0; i < count; i++) {
	 ...
	 * @param offset the position in <code>dest</code> where to start writing the samples of this buffer
	...
	public void copyTo(AudioBuffer dest, int offset, int count) {
		...
		if (count+offset > dest.getSampleCount()) {
			count = dest.getSampleCount()-offset;
		}
		int channelCount1 = getChannelCount();
		if (channelCount1 > dest.getChannelCount()) {
			channelCount1 = dest.getChannelCount();
		}
		for (int ch = 0; ch < channelCount1; ch++) {
			double[] thisChannel = getChannel(ch);
			double[] otherChannel = dest.getChannel(ch);
	 ...
	 * are written at <code>destOffset</code>. This AudioBuffer must be big
	 * enough to accomodate the samples.
	 * <p>
	...
	public void setSamplesFromBytes(byte[] input, int inByteOffset,
		...
		if (doubleOffset < 0 || frameCount < 0 || inByteOffset < 0) {
			throw new IllegalArgumentException(
					"AudioBuffer.setSamplesFromBytes: negative inByteOffset, doubleOffset, or frameCount");
		}
		if (inByteOffset + (frameCount * format.getFrameSize()) > input.length) {
			throw new IllegalArgumentException(
					"AudioBuffer.setSamplesFromBytes: input buffer too small.");
		}
		if (doubleOffset + frameCount > getSampleCount()) {
			throw new IllegalArgumentException(
					"AudioBuffer.setSamplesFromBytes: frameCount too large");
		}

		ConversionTool.byte2double(input, inByteOffset, channels, doubleOffset,
	...
	public void setSampleRate(double sampleRate) {
		if (sampleRate <= 0) {
			throw new IllegalArgumentException(
					"Invalid samplerate for AudioBuffer.");
		}
	...
	public double[] getChannel(int channel) {
		if (channel >= this.channelCount) {
			throw new IllegalArgumentException(
					"AudioBuffer: invalid channel number.");
		}
	 ...
	 */
	public void setDitherBits(double ditherBits) {
		if (ditherBits <= 0) {
			throw new IllegalArgumentException(
	...
	protected double getConvertDitherBits(int newFormatType) {
		...
		case DITHER_MODE_AUTOMATIC:
			doDither =
					(originalFormatType & ConversionTool.F_SAMPLE_WIDTH_MASK) > (newFormatType & ConversionTool.F_SAMPLE_WIDTH_MASK);
			break;
		case DITHER_MODE_ON:
			doDither = true;
			break;
		...
		return doDither ? ditherBits : 0.0;
	}
	...
	public static void main(String[] args) {
		...
		AudioBuffer ab = new AudioBuffer(2, SIZE, 44100.0);
