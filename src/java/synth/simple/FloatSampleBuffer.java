...
public class FloatSampleBuffer {
	...
	protected void init(int aChannelCount, int aSampleCount, float aSampleRate, boolean lazy) {
		if (aChannelCount<0 || aSampleCount<0) {
			throw new IllegalArgumentException(
			    "invalid parameters in initialization of FloatSampleBuffer.");
		}
		setSampleRate(aSampleRate);
		if (getSampleCount()!=aSampleCount || getChannelCount()!=aChannelCount) {
			createChannels(aChannelCount, aSampleCount, lazy);
		}
	}

	private void createChannels(int aChannelCount, int aSampleCount, boolean lazy) {
		this.sampleCount=aSampleCount;
		// lazy delete of all channels. Intentionally lazy !
		this.channelCount=0;
		for (int ch=0; ch<aChannelCount; ch++) {
			insertChannel(ch, false, lazy);
		}
		if (!lazy) {
			// remove hidden channels
			while (channels.size()>aChannelCount) {
	 ...
	 */
	public void reset(int aChannels, int aSampleCount, float aSampleRate) {
	...
	public void changeSampleCount(int newSampleCount, boolean keepOldSamples) {
		...
		if (keepOldSamples) {
			...
			for (int ch=0; ch<getChannelCount(); ch++) {
				...
				float[] newSamples=getChannel(ch);
	...
	public void insertChannel(int index, boolean silent, boolean lazy) {
		...
		if (physSize>virtSize) {
			...
			for (int ch=virtSize; ch<physSize; ch++) {
				float[] thisChannel=channels.get(ch);
	...
	public void mixDownChannels() {
		...
		int lSampleCount=getSampleCount();
		int lChannelCount=getChannelCount();
		for (int ch=lChannelCount-1; ch>0; ch--) {
			float[] thisChannel=getChannel(ch);
			for (int i=0; i<lSampleCount; i++) {
