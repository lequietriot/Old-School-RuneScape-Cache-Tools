...
public class SpriteDefinitions {
	...
	public static SpriteDefinitions[] decode(byte[] data) {
		...
		stream.offset = data.length - 7 - count * 8;
		...
		stream.offset = data.length - 7 - count * 8 - (palleteLength - 1) * 3;
	...
	public static SpriteDefinitions getSprite(Index index, int archiveId, int fileId) {
		byte[] data = Objects.requireNonNull(Objects.requireNonNull(index.archive(archiveId)).file(fileId)).getData();
	...
	public void method1536(int i_1) {
		...
		for (i_5 = 0; i_5 < height; i_5++)
			for (i_6 = 0; i_6 < width; i_6++) {
				i_7 = pixels[i_3] & 0xff;
				if (pallete[i_7] == 0)
					if ((i_6 > 0 && pallete[pixels[i_3 - 1] & 0xff] != 0) || (i_5 > 0 && pallete[pixels[i_3 - width] & 0xff] != 0))
						i_7 = i_2;
